...
<nav class="main-nav">
        <div class="contain-nav">
          ...
          <a href="/" class="logo">SIABINILA</a>
          
          <ul class="navlinks">
          <li><a href="<?= base_url('Dosen')?>" class="<?= \Config\Services::request()->uri->getSegment(2) == '' ? 'active'  : '' ?>"><i class="fa fa-home"></i> HOME</a></li>
            <li><a href="<?= base_url('Dosen/profil')?>" class="<?= \Config\Services::request()->uri->getSegment(2) == 'profil' ? 'active' : '' ?>"><i class="fa fa-user">&nbsp;</i><?= !session()->get('user') ? 'Login ' : session()->get('nama') ?></a></li>
          </ul>
        </div>  
      </nav>
</header>
